...
5. **Нет тестов** для дашборда и Digest API.
6. **Нет HTTPS** — будет в Фазе 4 с Nginx.
7. **Нет миграций БД** — alembic не настроен, при изменении схемы нужно удалять users.db.
8. **WebSocket стриминг** — код написан (шаг 21), но не отлажен. На первом компе заказчика uvicorn не подхватывал WS (проблема с окружением/браузером), на втором компе тест работает. Файлы шага 21 есть в репо но не активированы — чат работает через обычный POST.
9. **Docker контейнеры** — на компе заказчика запущены контейнеры (codeks) на порту 8000, поэтому дашборд перенесён на 9001.

---

## 11. WebSocket стриминг (шаг 21) — ОТЛОЖЕН

Код написан но не активирован. Включает:
...
Проблема: на основном компе заказчика WebSocket не подключается (readyState сразу 3). На другом компе с чистым venv — работает. Предположительно проблема в окружении или браузере первого компа.

Решено пропустить и вернуться позже.
...
- **Шаг 21**: WebSocket стриминг (отложен, код есть)
